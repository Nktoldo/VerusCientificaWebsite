...
export default function ProductsClient() {
      ...
      try {
        if (slug.length === 0) {
          ...
          if (products.length === 0) {
            ...
            // CORRIGIR: usar nova estrutura
            products = fallback.filter((p: any) => {
              // Nova estrutura: verificar array categorias
              if (p.categorias && Array.isArray(p.categorias)) {
                return p.categorias.includes(slug[0]);
              }
              // Estrutura legada (compatibilidade)
              return p.category === slug[0] && (!p.subcategory || p.subcategory === "");
            });
          }
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-white/70 z-50">
        <div className="flex flex-col items-center">
          <div className="w-12 h-12 sm:w-16 sm:h-16 border-4 border-blue-400 border-t-transparent rounded-full animate-spin mb-3 sm:mb-4"></div>
          <span className="text-blue-700 font-semibold text-base sm:text-lg">Carregando...</span>
        </div>
      </div>
    );
  }
  ...
  if (items.length === 0 && !loading) {
    return (
      <div className="flex flex-col gap-6 sm:gap-10 justify-center items-center px-4">
        <div className="text-center">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-700 mb-3 sm:mb-4">Nenhum item encontrado</h2>
          <p className="text-gray-500 text-sm sm:text-base">Não foi possível carregar os dados solicitados.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8 md:gap-10 justify-center items-center w-full">
      ...
      {slug.length > 0 && (
        <button 
          onClick={handleBack} 
          className="flex items-center gap-2 px-4 py-2 text-sm sm:text-base text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition-colors duration-200 mb-2 sm:mb-4"
        >
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3 md:gap-4 items-center justify-center px-4">
        {items.map((it) => {
      ...
      {/* Seção de produtos */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-8 lg:gap-12 xl:gap-16 items-start justify-center px-4 w-full max-w-7xl">
        {items.map((it) => {
          if (isKV(it) && typeof it.value === "object" && "title" in it.value && "price" in it.value) {
            ...
            return (
              <div key={key} onClick={() => handleProductClick(key)} className="w-full">
